...
public class ScopeDataManager {
	...
	private String GetCorrespondKindString(Integer kind) {
		switch (kind) {
		case 2^0:
	...
	private Integer ExtractKindAccordingToHint(Integer kinds, KindHint hintkind) {
		int low = hintkind.getLow();
		int high = hintkind.getHigh();
		return ((((1<<(high+1))-1)&kinds)>>low)<<low;
	}
	
	private VDataPoolManager ChooseManagerAccordingToKind(String kind)
	{
		switch (kind) {
		case VVarObjPoolManager.ConstantPool:
		case VVarObjPoolManager.VarOrObjPool:
	...
	private int GetKindRepresentation(String kind)
	{
		switch (kind) {
		case VVarObjPoolManager.ConstantPool:
			return 2^0;
	...
	public void AddDataNewlyUsed(String data, String kind, boolean isfielddeclare, boolean iscommondeclare) {
		VDataPoolManager vdpm = ChooseManagerAccordingToKind(kind);
		...
		if (newthing)
		{
			...
			kindval |= GetKindRepresentation(kind);
	...
	private String GetDataExactOffset(OneScope dataScope, String data, KindHint hintkind)
	{
		...
		Integer kind = ExtractKindAccordingToHint(kinds, hintkind);
		String kindstr = GetCorrespondKindString(kind);
		VDataPoolManager vdpm = ChooseManagerAccordingToKind(kindstr);
