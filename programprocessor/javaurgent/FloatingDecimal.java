	...
	public String toJavaFormatString() {
		...
		if (isExceptional) {
			...
			if (decExponent > 0 && decExponent < 8) {
				...
				result[i++] = '.';
				if (decExponent != 0) {
					JVM.unchecked_char_arraycopy(zero, 0, result, i, -decExponent);
					i -= decExponent;
				}
				JVM.unchecked_char_arraycopy(digits, 0, result, i, nDigits);
				i += nDigits;
				...
				result[i++] = '.';
				if (nDigits > 1) {
					JVM.unchecked_char_arraycopy(digits, 1, result, i, nDigits - 1);
					i += nDigits - 1;
				} else {
					result[i++] = '0';
				}
	...
	public FDBigInt mult(FDBigInt other) {
		...
		for (i = 0; i < this.nWords; i++) {
			...
			for (j = 0; j < other.nWords; j++) {
				p += ((long) r[i + j] & 0xffffffffL) + v * ((long) other.data[j] & 0xffffffffL); // UNSIGNED
																									// CONVERSIONS
																									// ALL
																									// 'ROUND.
				r[i + j] = (int) p;
		...
		if (diff != 0L) {
			...
			while (sum == 0L) {
				...
				if (sum != 0 && sum != 1) {
					throw new RuntimeException(
					/* #ifdef VERBOSE_EXCEPTIONS */
					/// skipped "Assertion botch: "+sum+" carry out of division
					/// correction"
					/* #endif */
					);
				}
