...
public final class MethodsParser {
	...
	public static final Predicate<Method> METHOD_IS_PUBLIC = (m) -> Modifier.isPublic(m.getModifiers()),
			METHOD_IS_GETTER = (m) -> m.getParameterCount() == 0
	...
	public static Stream<Method> streamOfExternalNoneSecretGetters(Class<?> clazz) {
		...
		return getMethods(clazz, METHOD_IS_PUBLIC.and(METHOD_IS_GETTER).and(METHOD_IS_EXTERNAL_AND_NONE_SECRET))
				.stream();
	}

	public static Stream<Method> streamOfExternalSetters(Class<?> clazz) {
		requireNonNull(clazz);
		return getMethods(clazz, METHOD_IS_PUBLIC.and(METHOD_IS_SETTER).and(METHOD_IS_EXTERNAL)).stream();
	}
	...
	public static External getExternalFor(final Method method, final Class<?> clazz) {
		...
		for (final Class<?> classCandidate : classCandidates) {
			try {
				return getExternalFor(classCandidate.getMethod(method.getName(), method.getParameterTypes()),
						classCandidate);
	...
	private static boolean isVisibleInGUI(final Method method, final Class<?> clazz) {
		requireNonNull(method);
		requireNonNull(clazz);
		return Optional.ofNullable(getExternalFor(method, clazz)).filter(External::isVisibleInGui).isPresent();
	}
	...
	private static boolean isExternalAndNoneSecret(final Method method, final Class<?> clazz) {
		requireNonNull(method);
		requireNonNull(clazz);
		return Optional.ofNullable(getExternalFor(method, clazz)).filter(e -> !e.isSecret()).isPresent();
	}
	...
	private static boolean isExternalAndSecret(final Method method, final Class<?> clazz) {
		requireNonNull(method);
		requireNonNull(clazz);
		return Optional.ofNullable(getExternalFor(method, clazz)).filter(External::isSecret).isPresent();
	}

	private static Map<String, Method> addMethods(Map<String, Method> methods, Class<?> clazz,
		...
		Stream.of(clazz.getDeclaredMethods()).filter(filter).forEach(m -> {
